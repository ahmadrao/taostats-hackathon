...
import ValidatorData from '@/utils/validators.json';
...
const TableComponent = () => {
  ...
  const columns = useMemo<ColumnDef<Validator>[]>(() => [

    {
      ...
      cell: (info) => {
        return <div className="flex gap-5">
          <Avatar imageUrl={info.row.original.validatorUrl} className="bg-[#191919]" />
    ...
    {
      accessorKey: 'network',
      header: () => <span className="text-table-teal">Network</span>,
      cell: info => {
  ...
  const data = useMemo(() => ValidatorData.data.validators.nodes.slice(0,5), []);
